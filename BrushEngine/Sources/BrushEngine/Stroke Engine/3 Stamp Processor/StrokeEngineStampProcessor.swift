...
extension StrokeEngineStampProcessor {
    
    struct Config {
        let stampGenerator: StrokeEngineStampGenerator
    ...
    struct Segment {
        var controlPointSamples: [Sample]
    ...
    struct SubSegmentEndpoint {
        var sample: Sample
        var noiseSample: NoiseSample
    }
    
}

// MARK: - StrokeEngineStampProcessor

struct StrokeEngineStampProcessor {
    ...
    private static func processSubSegment(
        ...
        while true {
            ...
            let noiseSample = try! SampleInterpolator
                .interpolate(
