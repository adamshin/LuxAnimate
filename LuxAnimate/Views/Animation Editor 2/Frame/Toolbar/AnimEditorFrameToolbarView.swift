...
class AnimEditorFrameToolbarView: PassthroughView {
    ...
    init() {
        ...
        addSubview(barView)
        addSubview(remainderContentView)
        
        barView.pinEdges([.top, .horizontal])
        barView.pinHeight(to: barHeight)
        
        remainderContentView.pinEdges([.bottom, .horizontal])
        remainderContentView.pin(.top, to: barView, toAnchor: .bottom)
        
        // Bar content
        let blurView = ChromeBlurView(
            overlayColor: .editorBarOverlay)
        
        barView.addSubview(blurView)
        blurView.pinEdges()
        blurView.backgroundColor = .editorBarShadow
        
        let shadow = UIView()
        shadow.backgroundColor = .editorBarShadow
        barView.addSubview(shadow)
