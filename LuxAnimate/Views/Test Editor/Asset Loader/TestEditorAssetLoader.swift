//
//  TestEditorAssetLoader.swift
//

import Metal

protocol TestEditorAssetLoaderDelegate: AnyObject {
    
    func onLoadAsset(_ loader: TestEditorAssetLoader)
    func onError(_ loader: TestEditorAssetLoader)
    
}

class TestEditorAssetLoader {
    
    private struct CancelLoadError: Error { }
    
    weak var delegate: TestEditorAssetLoaderDelegate?
    
    private let projectID: String
    
    private let loadQueue = DispatchQueue(
        label: "TestEditorAssetLoader.loadQueue",
        qos: .background)
